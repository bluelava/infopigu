...
“辟谷”原本是道家养生中的一种方法，核心不是盲目拒绝摄入，而是减少无效消耗，让身体回到更清洁、更敏锐的状态。

信息辟谷（InfoPigu）把这个概念借用到信息环境里：在信息过载时代，真正稀缺的不是内容总量，而是干净、有效、真正新增的信息。

因此，这个项目的目标不是做传统收藏夹，也不是做简单摘要，而是围绕“信息增量”工作。它的核心特点可以概括为三点：

- 抽取：只抓正文，尽量忽略视频壳层、按钮文案、作者信息、发布时间和平台噪音
- 去重：把当前页面和本地已读知识库比对，判断哪些内容你实际上已经看过
- 完全隐私：阅读记录和知识库优先保存在本地，插件只在白名单站点启用，云端调用由你自己配置

围绕这三点，Cognitive Delta / InfoPigu 会持续完成下面这些工作：
...
- 方式一：下载源代码，在本地编译后加载到 Chrome / Edge
- 方式二：直接下载编译好的插件 `.zip`，解压后加载到 Chrome
...
### 方式二：下载编译好的插件 ZIP 后安装

说明：Chrome / Edge 不能把 `.zip` 直接当作扩展安装，因此正确流程是“下载 zip -> 解压 -> 加载解压后的目录”。
...
如果你是从 GitHub Release 或别人分享的构建产物下载，也是同一个思路。

#### Step 2. 解压 ZIP

在 macOS / Linux 终端中可以执行：

```bash
unzip cognitive-delta-extension.zip -d /tmp/cognitive-delta-extension
```

Windows 下可以直接右键解压到某个目录。
...
#### Step 3. 在 Chrome 中安装解压后的插件

1. 打开 `chrome://extensions/`
2. 打开“开发者模式”
3. 点击“加载已解压的扩展程序”
4. 选择刚刚解压出来的目录，例如 `/tmp/cognitive-delta-extension`

#### Step 4. 在 Edge 中安装解压后的插件
